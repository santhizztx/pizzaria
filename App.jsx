...
// Preço segue a mesma regra que já existia (bordaPriceFor por pizza),
// igual pras 4 opções — o valor não muda conforme o sabor da borda.
const BORDA_OPTIONS = [
...
------------------------------------------------------------------- */
function bordaPriceFor(product) {
...
function OrderForm({ cart, onIncrement, onDecrement, onRemove, onBordaChange, formRef }) {
  ...
  function bordaLabelFor(item) {
    ...
    return `${BORDA_LABELS[item.borda] || item.borda} (+ ${formatBRL(bordaPriceFor(item.pizza))} cada)`;
  }
...
export default function App() {
  ...
  const cartTotal = Object.entries(cart).reduce((sum, [id, item]) => {
    ...
    const bordaExtra = !product.isDrink && item.borda !== "nenhuma" ? bordaPriceFor(product) : 0;
