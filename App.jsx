...
------------------------------------------------------------------- */
const BEBIDAS = [
  { id: "paulistinha", name: "Paulistinha 2 litros", price: 8.0, isDrink: true },
  { id: "coca-600-normal", name: "Coca-Cola 600ml Normal", price: 6.5, isDrink: true },
  { id: "coca-600-zero", name: "Coca-Cola 600ml Zero", price: 6.5, isDrink: true },
  { id: "coca-1l-casco", name: "Coca-Cola 1 litro (com casco/vasilhame)", price: null, isDrink: true },
  { id: "coca-2l-retornavel", name: "Coca-Cola 2 litros retornável", price: null, isDrink: true },
];
    ...
    <header className="pz-header">
      <div className="pz-header-inner">
        ...
        <nav className="pz-nav">
          ...
          <a href="#bebidas" onClick={(e) => onNavClick(e, "bebidas")}>Bebidas</a>
...
function OrderForm({ cart, onIncrement, onDecrement, onRemove, onBordaChange, formRef }) {
  ...
  const cartItems = Object.entries(cart)
    .map(([id, info]) => {
      const pizza = ALL_PRODUCTS.find((p) => p.id === id);
  ...
  function lineTotal(item) {
    const extra = !item.pizza.isDrink && item.borda !== "nenhuma" ? bordaPriceFor(item.pizza) : 0;
...
export default function App() {
  ...
  const cartTotal = Object.entries(cart).reduce((sum, [id, item]) => {
    const pizza = ALL_PRODUCTS.find((p) => p.id === id);
    if (!pizza) return sum;
    const bordaExtra = !pizza.isDrink && item.borda !== "nenhuma" ? pizza.bordaPrice ?? BORDA_PRICE : 0;
  ...
  return (
    <div className="pz-root">
      <style>{`
        ...
        #cardapio, #bebidas, #como-funciona, #pedido { scroll-margin-top: 132px; }
        @media (max-width: 560px) { #cardapio, #bebidas, #como-funciona, #pedido { scroll-margin-top: 128px; } }
        @media (max-width: 430px) { #cardapio, #bebidas, #como-funciona, #pedido { scroll-margin-top: 108px; } }
