...
import { Phone, User, Home, MapPin, StickyNote, Check, MessageCircle, ChevronRight } from "lucide-react";

/* ------------------------------------------------------------------
   DADOS DO CARDÁPIO
   Estrutura preparada para expansão futura: preço, imagem, tamanhos
   e adicionais já existem no modelo, só não são exibidos ainda.
------------------------------------------------------------------- */
const PIZZAS = [
  { id: "calabresa", name: "Calabresa", category: "Tradicionais", price: null, image: null, sizes: [], extras: [] },
  { id: "mussarela", name: "Mussarela", category: "Tradicionais", price: null, image: null, sizes: [], extras: [] },
  { id: "portuguesa", name: "Portuguesa", category: "Tradicionais", price: null, image: null, sizes: [], extras: [] },
  { id: "frango-catupiry", name: "Frango com Catupiry", category: "Tradicionais", price: null, image: null, sizes: [], extras: [] },
  { id: "marguerita", name: "Marguerita", category: "Especiais", price: null, image: null, sizes: [], extras: [] },
  { id: "quatro-queijos", name: "Quatro Queijos", category: "Especiais", price: null, image: null, sizes: [], extras: [] },
  { id: "pepperoni", name: "Pepperoni", category: "Especiais", price: null, image: null, sizes: [], extras: [] },
  { id: "vegetariana", name: "Vegetariana", category: "Especiais", price: null, image: null, sizes: [], extras: [] },
  { id: "bacon-cheddar", name: "Bacon com Cheddar", category: "Especiais", price: null, image: null, sizes: [], extras: [] },
  { id: "chocolate", name: "Chocolate com Morango", category: "Doces", price: null, image: null, sizes: [], extras: [] },
];

const CATEGORIES = ["Tradicionais", "Especiais", "Doces"];
...
/* ------------------------------------------------------------------
   CARDÁPIO
------------------------------------------------------------------- */
function Menu({ selectedId, onSelect }) {
  return (
    <section id="cardapio" className="pz-menu">
      ...
      <p className="pz-menu-sub">
        Preços, tamanhos e adicionais chegam em breve — por enquanto, escolha o sabor
        e siga com o pedido.
      </p>

      {CATEGORIES.map((cat) => (
        <div key={cat} className="pz-menu-category">
          <h3>{cat}</h3>
          <div className="pz-menu-grid">
            {PIZZAS.filter((p) => p.category === cat).map((p) => {
              const selected = p.id === selectedId;
              return (
                <div key={p.id} className={`pz-pizza-card${selected ? " is-selected" : ""}`}>
                  ...
                  <p className="pz-pizza-name">{p.name}</p>

                  <button
                    type="button"
                    className="pz-pizza-select"
...
------------------------------------------------------------------- */
function OrderBar({ pizza, onContinue }) {
    ...
    <div className="pz-orderbar">
      <div className="pz-orderbar-inner">
        ...
        <button className="pz-btn pz-btn-secondary" onClick={onContinue}>
          Continuar pedido <ChevronRight size={16} />
        </button>
      </div>
    </div>
  );
...
function OrderForm({ pizza, formRef }) {
  ...
  const requiredFields = ["nome", "telefone", "endereco", "numero", "bairro"];
  ...
  function validate() {
    ...
    if (!pizza) nextErrors.pizza = "Escolha uma pizza no cardápio";
  ...
  function buildMessage() {
    const linhas = [
      ...
      `- Bairro ${data.bairro}${data.cidade ? " — " + data.cidade : ""}`,
    ];
    if (data.complemento.trim()) linhas.push(`- Complemento: ${data.complemento}`);
    linhas.push("");
    linhas.push("Observações:");
    linhas.push(data.observacoes.trim() ? `- ${data.observacoes}` : "- Nenhuma");
    return linhas.join("\n");
  }
  ...
  return (
    <section id="pedido" className="pz-form-section" ref={formRef}>
      ...
      <div className="pz-form-wrap">
        <SpeechBubble>
          {!pizza
            ? "Escolha uma pizza no cardápio pra eu poder anotar seu pedido! 🍕"
            : sent
            ? "Show! Seu pedido tá a caminho do WhatsApp da pizzaria. 🎉"
            : "Só preencher os dados e mandar pro nosso zap!"}
        </SpeechBubble>

        <form className="pz-form" onSubmit={handleSubmit} noValidate>
          {pizza && (
            <div className="pz-form-selected">
              Pizza selecionada: <strong>{pizza.name}</strong>
            </div>
          )}
          {errors.pizza && <p className="pz-form-error pz-form-error-top">{errors.pizza}</p>}
          ...
          <div className="pz-form-grid">
            ...
            <label className="pz-field">
              ...
              <input name="telefone" value={data.telefone} onChange={handleChange} placeholder="(11) 99999-9999" />
              {errors.telefone && <em>{errors.telefone}</em>}
            </label>

            <label className="pz-field pz-field-wide">
              <span><Home size={15} /> Endereço</span>
              <input name="endereco" value={data.endereco} onChange={handleChange} placeholder="Rua, avenida..." />
              {errors.endereco && <em>{errors.endereco}</em>}
            </label>

            <label className="pz-field">
              <span>Número</span>
              <input name="numero" value={data.numero} onChange={handleChange} placeholder="123" />
              {errors.numero && <em>{errors.numero}</em>}
            </label>

            <label className="pz-field">
              <span><MapPin size={15} /> Bairro</span>
              <input name="bairro" value={data.bairro} onChange={handleChange} placeholder="Seu bairro" />
              {errors.bairro && <em>{errors.bairro}</em>}
            </label>

            <label className="pz-field">
              <span>Cidade</span>
              <input name="cidade" value={data.cidade} onChange={handleChange} placeholder="Opcional" />
            </label>

            <label className="pz-field pz-field-wide">
              <span>Complemento</span>
              <input name="complemento" value={data.complemento} onChange={handleChange} placeholder="Casa, apto, ponto de referência (opcional)" />
            </label>

            <label className="pz-field pz-field-wide">
              <span><StickyNote size={15} /> Observações do pedido</span>
              <textarea name="observacoes" value={data.observacoes} onChange={handleChange} placeholder="Sem cebola, borda recheada... (opcional)" rows={3} />
            </label>
          </div>
...
export default function App() {
  ...
  const selectedPizza = PIZZAS.find((p) => p.id === selectedId) || null;
    ...
    <div className="pz-root">
      <style>{`
        ...
        .pz-root {
          ...
          background-image:
            linear-gradient(45deg, rgba(143,44,35,0.06) 25%, transparent 25%, transparent 75%, rgba(143,44,35,0.06) 75%),
            linear-gradient(45deg, rgba(143,44,35,0.06) 25%, transparent 25%, transparent 75%, rgba(143,44,35,0.06) 75%);
        ...
        .pz-menu-category { text-align: left; margin-bottom: 30px; }
        .pz-menu-category h3 { color: var(--olive); font-size: 1.25rem; margin-bottom: 14px; border-bottom: 3px dashed var(--orange); padding-bottom: 6px; }
        ...
        .pz-pizza-img span { font-size: 0.72rem; font-weight: 700; letter-spacing: 0.03em; color: var(--ink); opacity: 0.6; }
        ...
        .pz-form-selected { background: var(--cream); border: 2px dashed var(--olive); border-radius: 10px; padding: 10px 14px; margin-bottom: 16px; font-size: 0.95rem; }
      ...
      <HowItWorks />
      <Menu selectedId={selectedId} onSelect={handleSelect} />
      <OrderForm pizza={selectedPizza} formRef={formRef} />
      <Reviews />
      <Footer />
      <OrderBar pizza={selectedPizza} onContinue={handleContinue} />
    </div>
  );
}
