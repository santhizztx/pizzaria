...
/* ------------------------------------------------------------------
   CARDÁPIO
------------------------------------------------------------------- */
function PizzaCard({ pizza, quantity, onAdd, onIncrement, onDecrement }) {
  return (
    <div className={`pz-pizza-card${quantity > 0 ? " is-selected" : ""}`}>
      ...
      <p className="pz-pizza-price">{formatBRL(pizza.price)}</p>
...
function Menu({ cart, onAdd, onIncrement, onDecrement, activeTab }) {
...
function OrderForm({ cart, onIncrement, onDecrement, onRemove, onBordaChange, formRef }) {
  ...
  const cartItems = Object.entries(cart)
    .map(([id, info]) => {
      const pizza = ALL_PIZZAS.find((p) => p.id === id);
      if (!pizza) return null;
      return { pizza, quantity: info.quantity, borda: info.borda };
    })
    .filter(Boolean);

  function bordaPriceFor(pizza) {
    return pizza.bordaPrice ?? BORDA_PRICE;
  }
  function lineTotal(item) {
    const extra = item.borda !== "nenhuma" ? bordaPriceFor(item.pizza) : 0;
  ...
  function bordaLabelFor(item) {
    ...
    const nome = item.borda === "catupiry" ? "Catupiry" : "Cheddar";
    return `${nome} (+ ${formatBRL(bordaPriceFor(item.pizza))} cada)`;
  }
  ...
  function buildMessage() {
    ...
    cartItems.forEach((item, i) => {
      ...
      linhas.push(`   Borda: ${bordaLabelFor(item)}`);
    ...
    <section id="pedido" className="pz-form-section" ref={formRef}>
      ...
      <div className="pz-form-wrap">
        ...
        <form className="pz-form" onSubmit={handleSubmit} noValidate>
              ...
              {cartItems.map((item) => (
                <div className="pz-cart-item" key={item.pizza.id}>
                  <div className="pz-cart-item-top">
                    <p className="pz-cart-item-name">{item.pizza.name}</p>
                    <button
                      type="button"
          ...
          <button type="submit" className="pz-btn pz-btn-primary pz-btn-wide">
...
export default function App() {
  // Carrinho: { [pizzaId]: { quantity, borda } }
  ...
  const cartTotal = Object.entries(cart).reduce((sum, [id, item]) => {
    const pizza = ALL_PIZZAS.find((p) => p.id === id);
    if (!pizza) return sum;
    const bordaExtra = item.borda !== "nenhuma" ? pizza.bordaPrice ?? BORDA_PRICE : 0;
    return sum + (pizza.price + bordaExtra) * item.quantity;
  }, 0);
  ...
  return (
    <div className="pz-root">
      <style>{`
        ...
        #cardapio, #como-funciona, #pedido { scroll-margin-top: 132px; }
        @media (max-width: 560px) { #cardapio, #como-funciona, #pedido { scroll-margin-top: 128px; } }
        @media (max-width: 430px) { #cardapio, #como-funciona, #pedido { scroll-margin-top: 108px; } }
        ...
        @media (max-width: 640px) { .pz-nav { gap: 10px; } .pz-nav a:not(.pz-nav-cta) { display: none; } }
        ...
        .pz-orderbar-inner { max-width: 1100px; margin: 0 auto; padding: 10px 20px; display: flex; align-items: center; justify-content: space-between; gap: 16px; }
        .pz-orderbar-info { display: flex; align-items: center; gap: 10px; color: var(--cream); }
        ...
        .pz-form-total {
          display: flex; align-items: center; justify-content: space-between; background: var(--red); color: var(--cream);
          border-radius: 10px; padding: 10px 16px; margin-bottom: 16px; font-size: 0.95rem; font-weight: 600;
        }
        .pz-form-total strong { font-family: 'Baloo 2', sans-serif; font-size: 1.15rem; }
      ...
      <Hero onCtaClick={() => scrollToId("cardapio")} />
      <HowItWorks />
      <Menu cart={cart} onAdd={handleAdd} onIncrement={handleIncrement} onDecrement={handleDecrement} activeTab={activeTab} />
      <OrderForm
        cart={cart}
